...
`pricing-mlops` queda como repo funcional/data science alineado con Cookiecutter Data Science. El script de publicacion de plataforma empaqueta un snapshot de ese repo en `pricing-mlops-source/`; el pipeline `mlops/azureml/pricing-mlops-pipeline.yml` usa `code: ../pricing-mlops-source` y muestra validacion, componentes de monitoreo AUTH y publicacion final. El command job `mlops/azureml/pricing-mlops-job.yml` queda como fallback.

La ruta AUTH monitoring derivada del notebook Avance 4 vive en `mlops/azureml/pricing-mlops-notebook-pipeline.yml`. Se selecciona con `MLOPS_JOB_TEMPLATE=notebook` y expone pasos intermedios para vigencia de recomendaciones, drift AUTH y decision operacional. No ejecuta el notebook completo como artifact operacional.
